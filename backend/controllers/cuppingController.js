...
const sendCuppingEmail = async ({ name, email, phone, country, course, message }) => {
  try {
    ...
    console.log("Sending cupping inquiry email with data:", JSON.stringify(emailData, null, 2));
    ...
    console.error("Unexpected error in sendCuppingEmail:", error);
    return {
      success: false,
...
const submitCuppingInquiry = async (req, res, next) => {
  try {
    ...
    try {
      // Log the inquiry
      console.log("New cupping course inquiry:", {
        name,
        email,
        phone,
      ...
      const emailResult = await sendCuppingEmail({
        name,
        email,
        phone,
        country,
      ...
      console.log('Cupping inquiry email sending result:', {
      ...
      return res.status(200).json({
        success: true,
        message: "Thank you for your inquiry! We'll contact you soon with course details and enrollment information.",
        emailSent: !emailResult.skipped,
      });

    } catch (emailError) {
      console.error('Error processing cupping inquiry:', emailError);

      // Even if email fails, we still want to acknowledge the form submission
      return res.status(200).json({
        success: true,
        message: "Thank you for your inquiry! We've received your submission and will contact you soon.",
        emailSent: false,
    ...
    console.error("Error in submitCuppingInquiry:", error);
    res.status(500).json({
      success: false,
      message:
